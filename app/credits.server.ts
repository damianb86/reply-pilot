...
function creditPurchaseBillingName(pkg: (typeof CREDIT_PACKAGES)[number], bonusCredits: number) {
  ...
  if (bonusCredits > 0) {
    return `Reply Pulse AI: Review Replies ${formatCreditCount(totalCredits)} credits (${formatCreditCount(pkg.credits)} + ${formatCreditCount(bonusCredits)} first-purchase bonus)`;
  }

  return `Reply Pulse AI: Review Replies ${formatCreditCount(pkg.credits)} credits`;
}
