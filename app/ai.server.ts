...
function personalityPrompt(replies: ImportedReply[], context: BrandVoiceContext = {}) {
  return [
    "Generate the Personality field for ReplyPulse, a Shopify app that drafts public replies to product reviews.",
    ...
    "Match the same structure as ReplyPulse's built-in presets.",
...
function previewPrompt(context: BrandVoiceContext) {
  ...
  return [
    "You are ReplyPulse. Generate one public reply to a product review using the merchant voice below.",
...
function reviewReplyPrompt(context: ReviewReplyContext) {
  ...
  return [
    "You are ReplyPulse. Generate one public merchant reply to a product review.",
    "Output the reply text only. No JSON, no Markdown, no label.",
    "Write like a thoughtful human merchant, not a review analyst.",
...
function reviewReplyRevisionPrompt(context: ReviewReplyRevisionContext) {
  ...
  return [
    "You are ReplyPulse. Edit an existing public merchant reply to a product review.",
